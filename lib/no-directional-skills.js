...
module.exports = function NoDirectionalSkills(dispatch) {
    ...
    command.add(['nds', 'nodirectionalskills'], (skill, sub, set) => {
        if (skill) {
            skill = skill.toLowerCase()
            if (skill == 'on') {
              enabled = true
              command.message(`[no-directional-skills] NDS enabled.`)
              return
            }
            if (skill == 'off') {
                enabled = false
                command.message(`[no-directional-skills] NDS disabled.`)
                return
              }
            skill = Number(skill)
            if (sub === undefined || isNaN(skill)) {
                command.message(`[no-directional-skills] NDS command paramater wrong or missing. Received command "nds ${skill}". Use command "/8 nds" for instructions.`)
                return
            }
            sub = sub.toLowerCase()
            if (set === undefined) {
        ...
        command.message('[no-directional-skills] How to use NDS commands:\n'
            + '"/8 nds {on/off}" example: "/8 nds off"\n'
